...
class Records extends \Magento\Framework\App\Action\Action {
    ...
    public function execute( ) { 
        ...
        foreach ( $fingerprintRecords as $key => $value ) { 
            ...
            $data[ ] = '<img id="output" style="width:150px;margin-top:10px" src="' . $image_url . '" />';
            ...
            $data[ ] = ( $value[ 'date_added' ] != "" ? $timezoneInterface->date( $value[ 'date_added' ] )->format( 'm/d/Y' ) : "-" );
    ...
    private function getFingerprintsData( ) {
        ...
        if( $order && isset( $order[ 0 ] ) && isset( $order[ 0 ][ 'column' ] ) && $order[ 0 ][ 'column' ] > -1 ) { 
            $orderByColumns = [ 0 => 'id', 1 => 'unique_upd_id', 2 => 'agent_id', 4 => 'name', 6 => 'first_name', 6 => 'date_added' ];
